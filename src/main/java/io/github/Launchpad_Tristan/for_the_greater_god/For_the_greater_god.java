...
public class For_the_greater_god implements ModInitializer {
    ...
    public static final Map<String, ChatFormatting> GOD_TEAMS = new LinkedHashMap<>();

    static {
        GOD_TEAMS.put("Zeus", ChatFormatting.YELLOW);
        GOD_TEAMS.put("Cronus", ChatFormatting.DARK_GREEN);
        GOD_TEAMS.put("Hades", ChatFormatting.DARK_RED);
        GOD_TEAMS.put("Poseidon", ChatFormatting.BLUE);
    }
    ...
    private void createTeams(MinecraftServer server) {
        ...
        for (Map.Entry<String, ChatFormatting> entry : GOD_TEAMS.entrySet()) {

            if (scoreboard.getPlayerTeam(entry.getKey()) == null) {

                PlayerTeam team = scoreboard.addPlayerTeam(entry.getKey());
